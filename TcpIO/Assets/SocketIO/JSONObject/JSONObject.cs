...
public class JSONObject {
	...
	public string str; //as object name.
	...
	//public JSONObject(string classname) { this.className = classname; }
	#region PARSE
		...
		if(!string.IsNullOrEmpty(str)) {
			...
			if(str.Length > 0) {
				if(string.Compare(str, "true", true) == 0) {
					...
					if (type == Type.OBJECT) {
						++offset;
						int classpos = str.IndexOf (':');
						className = str.Substring (offset + 1, classpos - offset - 2);
						offset += (classpos - offset + 1); //ignore :{ for class memebers..
					}
...
#pragma warning restore 219
	 ...
	 */
	// Default parameters fix
	public void Stringify(int depth, StringBuilder builder){ Stringify(depth, builder, false); }
	public void Stringify(int depth, StringBuilder builder, bool pretty) {	//Convert the JSONObject into a string
		//Profiler.BeginSample("JSONprint");
		if(depth++ > MAX_DEPTH) {
			Debug.Log("reached max depth!");
			return;
